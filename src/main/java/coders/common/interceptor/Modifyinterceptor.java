...
public class Modifyinterceptor implements HandlerInterceptor{
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if(smap ==null) {
			...
			if(PROJECT_NO!=null) {
				if(PROJECT_BOARD_NO!=null) {
					...
					map = projectService.selectProjectDetail(smap);
					int ch1 = Integer.parseInt(map.get("USER_NO").toString());
					int ch2 = Integer.parseInt(smap.get("USER_NO").toString());	
				...
				map = boardService.selectBoardDetail(smap);
				int ch1 = Integer.parseInt(map.get("USER_NO").toString());
				int ch2 = Integer.parseInt(smap.get("USER_NO").toString());	
		...
		PrintWriter printwriter = response.getWriter();
		printwriter.print("<script>alert('글수정은 작성자만 가능합니다'); location.href='../board/mainList.do'</script>");
		printwriter.flush();
		printwriter.close();
		return false;
	}
